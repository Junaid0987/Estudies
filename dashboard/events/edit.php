...
include "../database.php"; 
    ...
    if(isset($_POST['eventsedit'])){
      if(isset($_FILES["image"]["name"]) && !empty($_FILES["image"]["name"]) ){
        ...
        $save = $db->query("update `events` set `title` ='".$_POST['title']."',`description` = '".$_POST['description']."',`image`='".$filename."', `category` ='".$_POST['category']."', `start_date_time` ='".$_POST['start_date_time']."', `end_date_time`='".$_POST['end_date_time']."', `status`='".$_POST['status']."' where `id` = ".$_POST['id'] );
      }else{
        $save = $db->query("update `events` set `title` ='".$_POST['title']."',`description` = '".$_POST['description']."', `category` ='".$_POST['category']."', `start_date_time` ='".$_POST['start_date_time']."', `end_date_time`='".$_POST['end_date_time']."', `status`='".$_POST['status']."' where `id` = ".$_POST['id'] );
      }
      
      
        if($save){
            $address = $url."/events/index.php?msg="."events Updated Successfully";
    ...
    if(isset($_GET['id'])){
      $link = $db->query("select *from events where id =" . $_GET['id']);
      ...
      <div class="col-lg-12 col-xl-11">
        <div class="card text-black" style="border-radius: 25px;">
          <div class="card-body p-md-5">
                ...
                <form class="mx-1 mx-md-4" action="edit.php" method="post" enctype="multipart/form-data">
                  ...
                  <div class="d-flex flex-row align-items-center mb-4"> 
                    <div class="form-outline flex-fill mb-0">
                      ...
                      <input type="text" id="form3Example3c" value="<?php echo $data['description']  ?>" name="description"  class="form-control" required/>
                    </div>
                  </div>
                  ...
                  <div class="d-flex flex-row align-items-center mb-4">
                    <div class="row">
                      <div class="col-md-6">
                        <img class="img-fluid" src="<?php echo $url.'events/images/'.$data['image']?>" alt="">
                      </div>
                      <div class="col-md-6">
                        <div class="form-outline flex-fill mb-0">
                  ...
                  <div class="d-flex flex-row align-items-center mb-4">
                    <div class="form-outline flex-fill mb-0">
                      ...
                      <input type="datetime-local" id="form3Example3c" value="<?php echo $data['start_date_time']  ?>"  name="start_date_time" class="form-control" />
                    </div>
                  </div>
                  ...
                  <div class="d-flex flex-row align-items-center mb-4">
                    <div class="form-outline flex-fill mb-0">
                      ...
                      <input type="datetime-local" id="form3Example3c" value="<?php echo $data['end_date_time']  ?>"  name="end_date_time" class="form-control" />
                    </div>
                  </div>
            ...
            <input type="submit" name="eventsedit" value="Edit Event" class="btn btn-block btn-primary btn-lg px-5" />
          </form>
          </div>
    </div>
</div>
